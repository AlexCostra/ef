...
from util import buildMPNN_main, buildMPNN_multihot, llprint, multi_label_metric, ddi_rate_score, get_n_params, \
    buildMPNN, buildMPNN_ecfp
...
from models_gnn import GNN, DrugGCN,DrugSyergisticGCN,DrugGAT,DrugSyergisticGAT
...
parser.add_argument('--datadir', type=str, default="../data/", help='datadir')  # "../data_ordered/"
...
parser.add_argument('--MIMIC', type=int, default=3, help="mimic3 or mimic4")  # ddi_adj (131, 131)
# parser.add_argument('--MIMIC', type=int, default=4, help="mimic3 or mimic4")#ddi_adj (131, 131)
...
def compute_mahalanobis_distance(x, mu, sigma_inv):
        """计算马氏距离"""
        delta = x - mu  # 计算偏差
        dist = torch.sqrt(torch.sum(delta @ sigma_inv * delta, dim=1))  # 按行计算距离
        return dist

def eval(model: main_model, data_eval, voc_size, epoch, metric_obj: Metrics,drug_data):
    model.eval()
    ...
    for data_tensors in model.get_batch(data_eval, 128):
        ...
        cur_diag, cur_pro, cur_med_target, _, cur_len, jhjhjhjh = data_tensors
        result, _, _ ,_,_= model((cur_diag, cur_pro, jhjhjhjh, None), cur_len,mol_data=drug_data['mol_data'],average_projection=drug_data['average_projection'],substruct_data=drug_data['substruct_data'],ddi_mask_H=drug_data['ddi_mask_H'])
...
def main():
    ...
    if args.MIMIC == 4:
        ...
        substruct_smile_path = os.path.join(args.datadir, 'substructure_smiles_4.pkl')
    else:
        ...
        substruct_smile_path = os.path.join(args.datadir, 'substructure_smiles.pkl')
    ddi_synergistic_adj_path=os.path.join(args.datadir, '131.pkl')
    molecule_path = os.path.join(args.datadir, 'ndc2SMILES.pkl')
    device = torch.device('cuda:' + str(args.cuda) if args.cuda > -1 else 'cpu')

    y_true_synergistic=[1 for i in range(131)]
    y_true_synergistic = torch.Tensor(y_true_synergistic).to(device)
    y_true_ddi=[0 for i in range(131)]
    y_true_ddi = torch.Tensor(y_true_ddi).to(device)
    ddi_adj = dill.load(open(ddi_adj_path, 'rb'))
    print('ddi_adj', ddi_adj.shape)
    ddi_synergistic_adj=dill.load(open(ddi_synergistic_adj_path, 'rb'))
    ...
    #ddi_encoder = DrugGCN(128, 64, 64, 4)
    ddi_encoder = DrugGAT(128, 64, 64, 4,4,0.3)
    #ddi_synergistic_encoder = DrugSyergisticGCN(128, 64, 64, 4)
    ddi_synergistic_encoder = DrugSyergisticGAT(128, 64, 64, 4,4,0.3)
    ...
    model = main_model(voc_size, ddi_adj, ddi_synergistic_adj,encoder,
                       ddi_encoder,  # add ddi_encoder
                       # exist_ddi_list, # add exist ddi
                       ddi_synergistic_encoder,
                       emb_dim=args.dim,
                       device=device,
                       use_aug=use_aug,
                       # ehr_adj=ehr_sim,
                       ...
                       med2diag=med2diag,
                       med2pro=med2pro,
                       args=args,substruct_num=ddi_mask_H.shape[1],substruct_dim=64,
                 global_dim=64)
    ...
    if args.Test:
        ...
        result = []
        metrics = eval(model, data_test_tensors, voc_size, 0, metric_obj,drug_data)  # ddi_adj删了
        model.save_embedding()
    ...
    for epoch in range(epoch_begin, EPOCH):
        ...
        for cur_batch in model.get_batch(data_train_tensors, 16):
            cur_diag, cur_pro, cur_med_bce_target, cur_med_ml_target, cur_len, cur_med = cur_batch
            result, loss_ddi, _,synergistic_features,ddi_features= model((cur_diag, cur_pro, cur_med, cur_med_bce_target),
                                                                         cur_len,mol_data=drug_data['mol_data'],average_projection=drug_data['average_projection'],substruct_data=drug_data['substruct_data'],ddi_mask_H=drug_data['ddi_mask_H'])
            ...
            loss = 0.95* loss_bce + 0.05 * loss_multi+2*(loss1_synergistic+loss1_ddi)+loss_ddi
            if args.ddi:
                labellist = []
                ...
                if cur_ddi_rate > args.target_ddi:  # 如果当前ddi率大于目标ddi率，则加入ddi loss
                    ...
                    loss = beta * loss + (1 - beta) * loss_ddi+(loss1_synergistic+loss1_ddi)
        ...
        print()
        tic2 = time.time()
        ddi_rate, ja, prauc, avg_p, avg_r, avg_f1, avg_med = eval(model, data_eval_tensors, voc_size, epoch, metric_obj,drug_data)
